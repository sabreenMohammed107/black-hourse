<div class="box-body">
    ...
    <h3 class="card-title float-sm-left mb-2"><a data-toggle="modal" data-target="#add-callcenter"
    ...
    <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-resizable="true"
        ...
        <tbody>
            @foreach ($callcenter as $index => $row)
                <tr>
                    <td></td>
                    <td>{{ $index + 1 }}</td>
                ...
                <td>{{$row->round->round_no ?? ''}}</td>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <a data-toggle="modal" data-target="#edit-callcenter{{ $row->id }}"
                                class="btn btn-default"><i class="fa fa-edit" title="تعديل"></i></a>
                            <button type="button" class="btn btn-default" data-toggle="modal"
                                data-target="#del-callcenter{{ $row->id }}"><i class="fa fa-times" title="حذف"></i></button>
                        </div>
                    </td>
                    <!-- Add Exception Modal -->
                    <div class="modal fade dir-rtl" id="edit-callcenter{{ $row->id }}" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <form action="{{ route('callcenter.update', $row->id) }}" method="post">

                                    @method('PUT')
                                    @csrf
                                    <input type="hidden" name="student_id" value="{{ $studentSS->id }}">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" name="round_id"
                                                        ...
                                                        @foreach ($folowRounds as $round)
                                                            <option value="{{ $round->id }}"
                                                                {{ $row->round_id == $round->id ? 'selected' : '' }}>
                                                                {{ $round->course->name ?? '' }} /{{ $round->round_no ?? '' }} </option>
                    ...
                    <!-- Delete Modal -->
                    <div class="modal modal-danger" id="del-callcenter{{ $row->id }}" tabindex="-1" role="dialog"
                        aria-labelledby="exampleModalLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <form action="{{ route('callcenter.destroy', $row->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <div class="modal-content">
...
<!-- Add fullowup Modal -->
<div class="modal fade dir-rtl" id="add-callcenter" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-light">
                ...
                <div class="modal-body text-center">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label>مجموعة</label>
                                <select class="form-control select2" name="round_id" style="width: 100%;">
                                    <option selected="selected">...</option>
