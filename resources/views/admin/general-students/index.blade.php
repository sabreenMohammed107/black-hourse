...
@section('content')
    ...
    <div class="box">
        <div class="box-header">
            ...
            <a href="{{ route('general-students.create') }}" class="btn btn-info btn-lg pull-right"> اضافة </a>
                    ...
                    <tbody>
                        @foreach ($rows as $index => $row)
                            <tr>
                                ...
                                <td>{{$row->funnel->sale_funnel ?? ''}}</td>
                                <td>{{$row->age}}</td>
                                <td>{{date('d-m-Y', strtotime($row->register_date))}}</td>
                                <td>{{$row->status->round_status ?? ''}}</td>
                                <td>
                                    <div class="btn-group">
                                        <a href="{{ route('general-students.show', $row->id) }}" class="btn btn-default"><i
                                                class="fa fa-eye" title="عرض"></i></a>
                                        <a href="{{ route('general-students.edit', $row->id) }}" class="btn btn-default"><i
                                ...
                                <div class="modal modal-danger" id="del{{ $row->id }}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <form action="{{ route('general-students.destroy', $row->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
